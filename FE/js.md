...
- `replaceAll` 메서드가 문자열에 추가되고, `any` 메서드가 프라미스에 추가되었습니다.
- `||=`, `&&=`, `??=` 논리 할당 연산자가 추가되었습니다.
...
- 값이 기록된 `Environment Record`와 `outer`라는 외부 렉시컬 환경을 가리키는 포인터로 구성되어 있습니다.
...
- 실행컨텍스트가 생성될 때, 렉시컬 스코프 내의 선언이 끌어올려지는 것을 의미합니다.
- var는 선언과 초기화가 동시에 일어나면서 호이스팅 관련 문제를 유발합니다.

### undefined vs null

- 두 타입 모두 값이 없다는 의미를 갖고 있습니다.
- undefined는 식별자로 쓸 수 있어서 변수를 할당 할 수 있지만 null은 불가능합니다.

### 화살표 함수 vs 일반 함수

- 일반 함수는 실행마다 this가 바인딩 되지만 화살표 함수는 this가없어 상위 스코프의 this를 가리킵니다.
- 화살표 함수는 프로토타입 속성이 없어 생성자 함수로 사용할 수 없습니다.
